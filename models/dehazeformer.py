...
#
#     def forward(self, x):
#         y = self.avg_pool(x) # 4 * 96 * 64 * 64
#         y = self.ca(y)  # 4 * 96 * 1 *1
#         x = x * y   # 4 * 96 * 64 *64
#
#         y = self.pa(x) # 4 * 96 * 64 * 64
#         x = x * y  # 4 * 1 * 64 * 64
#
#         return x  # 4 * 96 * 64 * 64
...
class DehazeFormer(nn.Module):
    def __init__(self, in_chans=3, out_chans=4, window_size=8,
        ...
        # self.unified_attention = UnifiedAttention(channel=96)
        self.conv1 = nn.Conv2d(in_channels=24, out_channels=96, kernel_size=1)
        self.conv2 = nn.Conv2d(in_channels=48, out_channels=96, kernel_size=1)
        self.dyhead = DyHead(in_channels=96, out_channels=96,num_blocks=1)
    ...
    def forward_features(self, x):
        x = self.patch_embed(x)  # 8 * 3 * 256 * 256
        x = self.layer1(x)  # 8 * 24 * 256 * 256
        skip1 = x  # 8 * 24 * 256 * 256
        outs0 = self.conv1(skip1)

        x = self.patch_merge1(x)  # 8 * 24 * 256 * 256
        x = self.layer2(x)  # 8 * 48 * 128 * 128
        skip2 = x  # 8 * 48 * 128 * 128
        outs1 = self.conv2(skip2)

        x = self.patch_merge2(x)  # 8 * 48 * 128 * 128
        x = self.layer3(x)  # 8 * 96 * 64 * 64
        outs = [outs0, outs1, x]
        x = self.dyhead(outs)

        # x = self.unified_attention(x)
        x = self.patch_split1(x)  # 8 * 96 * 64 * 64
        # x = self.unified_attention(x)

        x = self.fusion1([x, self.skip2(skip2)]) + x  # 8 * 48 * 128 * 128
        x = self.layer4(x)  # 8 * 48 * 128 *128
        x = self.patch_split2(x)  # 8 * 48 * 128 *128

        x = self.fusion2([x, self.skip1(skip1)]) + x  # 8 * 24 * 256 * 256
        x = self.layer5(x)  # 8 * 24 * 256 * 256
        # x = self.unified_attention(x)
        x = self.patch_unembed(x)  # 8 * 24 * 256 * 256
        return x  # 8 * 4 * 256 * 256
